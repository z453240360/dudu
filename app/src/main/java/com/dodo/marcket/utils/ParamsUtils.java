...
import com.dodo.marcket.bean.basebean.ParamsBeam;
...
public class ParamsUtils {
    ...
    private static String  app_key =  "appid1236587459";
    ...
    //直接Map转String
    public static RequestBody getParams(String beans, String name) {
        HashMap<String, Object> param = getPublicParams(name, beans);
        param.put("sign", buildSign(param, Constant.sign));
        RequestBody body = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), new Gson().toJson(param));
        return body;
    }
    ...
    //不带token
    public static RequestBody getParams(PhoneBean phoneBean) {
        HashMap<String, Object> param = getPublicParams(new Gson().toJson(phoneBean));
        param.put("sign", buildSign(param, Constant.sign));
        RequestBody body = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), new Gson().toJson(param));
        return body;
    }
    ...
    //公共参数，不带token
    private static HashMap<String, Object> getPublicParams(String data) {
        HashMap<String, Object> param = new HashMap<>();
        param.put("data", data);
        param.put("version", version);
        param.put("format",format);
